...
describe('Card.vue', () => {
	test('renders a card when passed valid data', async () => {
		...
		expect(wrapper.find('.type').text()).toBe('Type: Creature');
		expect(wrapper.find('.set').text()).toBe('Set: Core Set');
		expect(wrapper.find('.description').text()).toBe('Description: Prophecy, Guard. Last Gasp: Adoring Fan will return. Adoring Fan is immune to Silence.');
	});
	test('renders defaults when no data is passed', async () => {
		...
		expect(wrapper.find('.type').text()).toBe('Type: N/A');
		expect(wrapper.find('.set').text()).toBe('Set: N/A');
		expect(wrapper.find('.description').text()).toBe('Description: N/A');
	});
});
